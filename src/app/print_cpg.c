		...
		for (uint32_t pipe = 0; pipe < pipes_per_me; ++pipe) {
			...
			for (uint32_t queue = 0; queue < queues_per_pipe; ++queue) {
				...
				if (read_banked_reg(asic, "mmCP_GFX_HQD_ACTIVE") & 0x1) {
					...
					uint32_t pq_base_lo = read_banked_reg(asic, "mmCP_GFX_HQD_BASE");
					uint32_t pq_base_hi = read_banked_reg(asic, "mmCP_GFX_HQD_BASE_HI");
					uint64_t pq_base = ((((uint64_t)pq_base_hi) << 0x20) | pq_base_lo) << 0x8;
					uint32_t pq_rptr = read_banked_reg(asic, "mmCP_GFX_HQD_RPTR");
					uint32_t pq_wptr;
					if (asic->family < FAMILY_AI) {
						pq_wptr = read_banked_reg(asic, "mmCP_GFX_HQD_WPTR");
					} else {
						uint32_t pq_wptr_lo = read_banked_reg(asic, "mmCP_GFX_HQD_WPTR");
						uint32_t pq_wptr_hi = read_banked_reg(asic, "mmCP_GFX_HQD_WPTR_HI");
